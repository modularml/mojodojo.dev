...
Yes, Python packages perform cyclic imports and so we had to support that.
...
`FloatLiteral` is backed by `Float64`, the mantissa width is 52.
...
We also support an optional + result type as well for the usecases that benefit from it, e.g. functional patterns, although we are missing some support in the generics system to do that right now.
...
It's a false dichotomy in my opinion based on how those systems work. Mojo already supports great integration with Python and has a more powerful ownership system than Rust but lifetimes are not finished yet, so that isn't comparable yet.
...
We have experienced slow and overly complex type systems and too much sugar as you're pointing out. We've learned a lot from it, and the conclusion is "don't do it again".
...
Yes, you can do this in two ways: first any normal function may be used at compile time using `alias x = func()`.  No need to duplicate all math that works on ints between comptime and not, and no need to explicitly label everything as being constexpr capable

Second, runtime functions are also able to have “parameter results”, but it is mainly useful when returning parameterized capabilities from run time functions that are selected through auto tuning. This is an exotic power user feature, not the sort of thing I’d expect most mojo programmers to want to care about
...
Mojo now has arbitrary precision literals
...
We're still focusing on nailing down having packages first, but later on thinking about distribution and sharing will be important to building an ecosystem.
...
On the implementation, we'll need some work to build out `help(object)` and `help(Int)` (where Int is a struct, not a class).  I don't see us prioritizing that in the near term, but it is super important for us to do that over time.  We have ways to do that without adding a field to Int 🙂 etc, so that should be fine.
...
One nice thing about `alias` is that it is more obvious for the trivial cases like alias my_magic = 12312 or alias Int8 = SIMD[DType.int8, 1]. That doesn't make it the right thing, but it is a nice thing.
...
Nested functions (currently wired up, but have a few issues given lifetimes are not here yet). I'd like `@parameter` to go away on the nested functions eventually too. Existing Python lambda syntax, which is sugar, we need to support type annotations here.

Lower priority, but I think we're likely to explore possibly implementing more flexible/general/ergonomic light-weight closures like Scala3 => syntax
...
This is mostly just a placeholder for now. This has known problems and will be reworked now that `traits` are in. One problem with AnyType is that we will need to decide if it is implicitly copyable/movable, if that is trivial, etc.  There are lots of properties we'll want to be able to express elegantly; none of this has been designed, but there is a lot of prior art in rust/swift/haskell/etc.
...
We have to implement array bound checking for our array/slice types, we just haven't solidified them yet.
...
Check out the [blog post here](https://www.modular.com/blog/what-is-loop-unrolling-how-you-can-speed-up-mojo)
...
[utils.loop.unroll](https://docs.modular.com/mojo/stdlib/utils/loop#unroll) performs the same loop unrolling functionality as library functions. There are a few differences between using library function of unroll and decorator `@unroll`` are:
...
we run benchmarks against ONNX Runtime, and in our tests, the MAX Engine is almost always faster than it. The numbers are on the public dashboard at <https://performance.modular.com/>.
...
Mojo already gives a couple warnings that suggest better things to do. That said, the compiler isn't good at pointing out larger design pattern changes, for this I think we'll have LLM based tools outside the compiler itself. The UI is much better for explaining things in that context.
...
Mojo also has a more advanced ownership system than Rust or Swift, which is also pretty cool if you're into such things. Check out the programmer's manual here for more info: <https://docs.modular.com/mojo/manual/values/>
...
Check the [docs here](https://github.com/modularml/mojo/tree/nightly/stdlib)
...
Update: The stdlib is [now open source](https://github.com/modularml/mojo/tree/nightly/stdlib)
...
It will be open-sourced more over time, because Modular on its own isn't big enough to build the whole ecosystem Unlike Apple and Swift.
...
The MAX Engine will be different, and I expect/hope it to always be the best state of the art implementation of this stuff, but it can do so without trying to hold back other efforts!
...
Modular's building what we called the MAX engine, people are familiar with PyTorch and TensorFlow and these machine learning frameworks that provide APIs, underneath the covers there's a lot of technology for getting things onto a CPU and GPU through things like CUDA. And so our engine fits at that level of the stack, the cool thing about it particularly when you're deploying, is that it talks to a lot of different hardware.
